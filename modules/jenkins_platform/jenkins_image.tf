...
locals {
  ecr_endpoint = split("/", aws_ecr_repository.jenkins_master.repository_url)[0]
}


resource "aws_ecr_repository" "jenkins_master" {
...
data "template_file" jenkins_configuration_def {
  ...
  vars = {
    ecs_cluster_fargate       = aws_ecs_cluster.jenkins_master.arn
    ecs_cluster_fargate_spot  = aws_ecs_cluster.jenkins_agents.arn
    cluster_region            = local.region
    jenkins_cloud_map_name    = "master.${var.name_prefix}"
    jenkins_master_port       = var.jenkins_master_port
    jnlp_port                 = var.jenkins_jnlp_port
    agent_security_groups     = aws_security_group.jenkins_master_security_group.id
...
docker build -t ${aws_ecr_repository.jenkins_master.repository_url}:latest ${path.module}/docker/ && \
docker push ${aws_ecr_repository.jenkins_master.repository_url}:latest
EOF
  }
}